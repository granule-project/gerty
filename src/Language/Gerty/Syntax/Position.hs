...
module Language.Gerty.Syntax.Position
  ( -- * Positions
    Position
  , PositionWithoutFile
  , Position'(..)
...
instance Pretty a => Pretty (Position' (Maybe a)) where
  pprint (Pn Nothing  _ l c) = pprint l <> text "," <> pprint c
  pprint (Pn (Just f) _ l c) =
    pprint f <> colon <> pprint l <> text "," <> pprint c

instance Pretty PositionWithoutFile where
  pprint p = pprint (p { srcFile = Nothing } :: Position)

instance Pretty IntervalWithoutFile where
  pprint (Interval s e) = start <> text "-" <> end
    where
      sl = posLine s
      el = posLine e
      sc = posCol s
...
instance Pretty a => Pretty (Interval' (Maybe a)) where
  pprint i@(Interval s _) = file <> pprint (setIntervalFile () i)
    where
      file :: Doc
      file = case srcFile s of
               Nothing -> empty
               Just f  -> pprint f <> colon

instance Pretty a => Pretty (Range' (Maybe a)) where
  pprint r = case rangeToIntervalWithFile r of
    Nothing -> empty
    Just i  -> pprint i

instance (Pretty a, HasRange a) => Pretty (PrintRange a) where
  pprint (PrintRange a) = pprint a <+> parens (text "at" <+> pprint (getRange a))
