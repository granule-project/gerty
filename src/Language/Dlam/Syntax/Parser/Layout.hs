...
-}
module Language.Dlam.Syntax.Parser.Layout
...
import Language.Dlam.Syntax.Parser.LexActions
...
{-| Executed for the first token in each line (see 'Language.Dlam.Syntax.Parser.Lexer.bol').
    ...
    - /to the left/ :
        ...
        'Language.Dlam.Syntax.Parser.Lexer.bol' state).

    - /same column/ :
        Exit the 'Language.Dlam.Syntax.Parser.Lexer.bol' state and return a virtual semi
        colon.

    - /to the right/ :
        Exit the 'Language.Dlam.Syntax.Parser.Lexer.bol' state and continue lexing.
...
{-| This action is only executed from the 'Language.Dlam.Syntax.Parser.Lexer.empty_layout'
    state. It will exit this state, enter the 'Language.Dlam.Syntax.Parser.Lexer.bol' state,
...
{-| Start a new layout context. This is one of two ways to get out of the
    'Language.Dlam.Syntax.Parser.Lexer.layout' state (the other is 'openBrace'). There are
    two possibilities:
    ...
    block so we enter the 'Language.Dlam.Syntax.Parser.Lexer.empty_layout' state. In both
