...
substAbs :: (Monad m, HasTyVal v (Maybe a) (Expr e), HasBinderMap m Identifier v, Freshenable m Identifier, Substitutable m Identifier (Expr e)) => (Identifier, Expr e) -> Abstraction e -> m (Abstraction e)
substAbs s ab = do
  let v = absVar ab
  v' <- freshen v
